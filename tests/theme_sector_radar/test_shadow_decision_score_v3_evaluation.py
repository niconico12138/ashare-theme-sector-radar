"""Tests for shadow decision score v3 evaluation."""
...
from evaluate_shadow_decision_score_v3 import (
...
class TestScoreComparison:
    ...
    def test_comparison_has_metrics(self):
        records = _make_records(30)
        result = compute_score_comparison(records, "test")
        for key in ["production", "shadow_v3"]:
            data = result[key]
...
class TestRegimeComparisons:
    ...
    def test_regime_has_metrics(self):
        records = _make_records(30, "broad_up") + _make_records(30, "broad_down")
        result = compute_regime_comparisons(records)
        for regime in ["broad_up", "broad_down"]:
            data = result[regime]
            assert "production" in data
            assert "shadow_v3" in data


# ======================================================================
# Test: V3 Improved Check
# ======================================================================

class TestV3Improved:
    def test_improved_when_better_gap(self):
        overall = {
            "production": {"top_bottom_gap": 0.5, "consistency": 50.0},
            "shadow_v3": {"top_bottom_gap": 0.8, "consistency": 50.0},
        }
        assert _check_v3_improved(overall) is True

    def test_improved_when_better_consistency(self):
        overall = {
            "production": {"top_bottom_gap": 0.5, "consistency": 50.0},
            "shadow_v3": {"top_bottom_gap": 0.5, "consistency": 55.0},
        }
        assert _check_v3_improved(overall) is True
    ...
    def test_not_improved_when_worse(self):
        overall = {
            ...
            "shadow_v3": {"top_bottom_gap": 0.5, "consistency": 50.0},
        }
        assert _check_v3_improved(overall) is False
...
class TestMarkdown:
    def test_markdown_structure(self):
        ...
        overall = {
            ...
            "shadow_v3": {"top_bottom_gap": 0.6, "hit_rate_diff": 6.0, "spearman_corr": 0.04, "consistency": 56.0},
        }
        windows = {"20d": overall, "40d": overall}
        regimes = {"broad_up": overall, "broad_down": overall, "mixed": overall}
        v3_dist = {"min": 20, "max": 80, "mean": 50, "spread": 60, "unique_count": 50}

        md = generate_markdown(coverage, overall, windows, regimes, v3_dist, "test_range")
        assert "Shadow Decision Score V3 Evaluation" in md
...
class TestProductionChange:
    def test_always_false_in_markdown(self):
        ...
        overall = {"sample_count": 100, "production": {}, "shadow_v3": {}}
