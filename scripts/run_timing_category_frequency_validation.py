#!/usr/bin/env python3
"""Compare paper-only intraday timing factor research on 5m and 1m data."""
...
def run_timing_category_frequency_validation(
    *,
    category: str,
    ...
    if category not in TIMING_FACTOR_CATEGORIES:
        raise ValueError(f"Unsupported timing factor category: {category}")
    common = {
        "candidate_files": [],
        "start": start,
        "end": end,
    ...
    report = {
        "schema_version": "timing_category_frequency_validation.v1",
        "category": category,
        "as_of": as_of,
        ...
        "comparison": compare_frequency_factor_reports(report_5m, report_1m, category=category),
    }
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_category = category.replace("/", "_")
    json_path = output_dir / f"{safe_category}_frequency_validation_{as_of}_{snapshot_label}.json"
    markdown_path = output_dir / f"{safe_category}_frequency_validation_{as_of}_{snapshot_label}.md"
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {report['category']} Frequency Validation",
        "",
    ...
    if not comparison["eligible_factor_ids"]:
        lines.append(f"| None | - | - | No 5m `{report['category']}` factor was promoted. |")
    return "\n".join(lines) + "\n"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run paper-only 5m then 1m timing category validation")
    parser.add_argument("--category", required=True, choices=TIMING_FACTOR_CATEGORIES)
