# run_daily_degraded_fixture.ps1
# Degraded Fixture Test script
# Uses minimal fixture data to validate degraded report pipeline
...
try {
    ...
    Write-Host "============================================================"
    Write-Host "Theme Sector Radar - Degraded Fixture Test" -ForegroundColor Yellow
    Write-Host "============================================================"
    ...
    # Output summary
    Write-Host "------------------------------------------------------------"
    Write-Host "Degraded Test Result" -ForegroundColor Yellow
    Write-Host "------------------------------------------------------------"

    if ($Status -eq "degraded") {
        Write-Host "Status: $Status (expected for minimal fixture)" -ForegroundColor Yellow
    } elseif ($Status -eq "ok") {
    ...
    if ($Process.ExitCode -ne 0) {
        Write-Host ""
        Write-Host "ERROR: Test failed with exit code $($Process.ExitCode)" -ForegroundColor Red
    ...
    Write-Host ""
    Write-Host "Degraded Fixture Test completed!" -ForegroundColor Green
